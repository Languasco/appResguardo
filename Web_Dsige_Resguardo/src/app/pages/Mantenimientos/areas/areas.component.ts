...
export class AreasComponent implements OnInit {
  ...
  constructor(private alertasService : AlertasService, private spinner: NgxSpinnerService, private loginService: LoginService,private funcionGlobalServices : FuncionesglobalesService, private funcionesglobalesService : FuncionesglobalesService, private areaService : AreaService, private uploadService : UploadService) {         
 ...
 editar({   id_Servicios, nombreServicio, estado}){
   this.flag_modoEdicion=true;
   this.formParams.patchValue({ "id_Servicios" : id_Servicios, "nombreServicio" :  nombreServicio ,  "estado" : estado, "usuario_creacion" : this.idUserGlobal });
   setTimeout(()=>{ // 
    $('#modal_mantenimiento').modal('show');
  },0);
