...
export class PersonalComponent implements OnInit {
  ...
  constructor(private alertasService : AlertasService, private spinner: NgxSpinnerService, private loginService: LoginService, private funcionesglobalesService : FuncionesglobalesService, private personalService :PersonalService, private uploadService : UploadService) {         
 ...
 getCargarCombos(){ 
    this.spinner.show();
    combineLatest([ this.personalService.get_estados(), this.personalService.get_empresas(), this.personalService.get_tipoDoc(), this.personalService.get_cargo(),  this.personalService.get_distritos() ]).subscribe( ([_estados, _empresas, _tipoDocumentos, _cargos ,_distritos ])=>{
   ...
   if ( this.flag_modoEdicion==false) { //// nuevo  
     ...
     this.personalService.set_savePersonal(this.formParams.value).subscribe((res:RespuestaServer)=>{
       ...
       if (res.ok ==true) {    
         ...
         console.log(res.data[0].id_Personal)
 ...
 editar({ id_Personal, id_Empresa, id_TipoDoc, nroDocumento_Personal, apellidos_Personal, nombres_Personal, id_Cargo , direccion_Personal , id_Distrito , estado }){
   this.flag_modoEdicion=true;
   setTimeout(()=>{ // 
    $('#modal_mantenimiento').modal('show');
  },0);
